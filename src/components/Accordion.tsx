...
const Accordion: FunctionComponent<AccordionProps> = ({ items, defaultOpenedItemIndex }) => {
  ...
  function getDefaultOpenedItemIndex() {
    const isValidIndex = (n: number | undefined) => n !== undefined && 0 <= n && n <= items.length - 1;
    if (isValidIndex(defaultOpenedItemIndex)) return defaultOpenedItemIndex;

    return UNDEFINED_ACCORDION_ITEM_INDEX;
  }

  const [currentIdx, setCurrentIdx] = useState(getDefaultOpenedItemIndex());
  const btnOnClick = (idx: number) => setCurrentIdx((currentValue) => (currentValue !== idx ? idx : UNDEFINED_ACCORDION_ITEM_INDEX));
